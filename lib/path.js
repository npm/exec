...
const getBinPaths = ({ start, top = '/' }) => {
  ...
  const binPaths = [
    path.resolve(current, 'node_modules/.bin'),
  ]
...
// return a new env object where all of the original cases of PATH are set to
// a string combining the bin directories with all of the existing path values
const setPath = ({ env, start, top }) => {
  ...
  // to preserve what the user probably expects, we'll set all of the same
  // environment variables they gave us to the fully hydrated path
